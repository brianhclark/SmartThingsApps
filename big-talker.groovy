		...
		if (!(settings.speechDeviceDefault == null) || !(customSpeechDevice == null)) {
			...
			if (state.ableToTalk){
				...
				currentSpeechDevices.each(){
					LOGDEBUG("TALK(${evt.name})|mP| attrs=${attrs} --3687")
					def currentStatus = ""
                    try {
					...
					if (!(currentTrack == null)){
						...
						if (!(currentTrack?.status == null)) { 
                        	LOGTRACE("Current Track has Status")
                        	LOGTRACE("TALK(${evt.name})|mP| ${it.displayName} | Current Status: ${currentStatus}, CurrentTrack: ${currentTrack}, CurrentTrack.Status: ${currentTrack.status}.") 
                        }
						if (currentTrack?.status == null) { 
                        	LOGTRACE("Current Track has NO Status")
                        	LOGTRACE("TALK(${evt.name})|mP| ${it.displayName} | Current Status: ${currentStatus}, CurrentTrack: ${currentTrack}.") 
                        }
